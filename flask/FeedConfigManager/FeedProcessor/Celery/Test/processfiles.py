...
class DatabaseTask(Task):
    ...
    def MasterFeedDefinitions(self):
        if self._db is None:
            self._db = Couchbase.connect(bucket=DATABASENAME)
            _MasterFeedDefinitions = self._db.get(
                DOCUMENTTYPEKEYFORMAT.format(masterTenantid, documentType)).value
...
def startProcess(tenantid, documentType):
    ...
    cbDocument = startProcess.cb.get(
        DOCUMENTTYPEKEYFORMAT.format(tenantid, documentType)).value
    if (cbDocument):
            ...
            if (trlRowCount == (len(lines) - 2)):
                if (trlRowCount > 0):
                        ...
                        if infile.find(k) > 0:
                            ...
                            for lineno, line in enumerate(lines):
                                if line.startswith('HDR'):
                                    ...
                                    try:
                                        insertLine(
                                            infile, tenantid, documentType, feedtype, rowkeyschema, rowschema, lineno, line)
                                        logger.debug(line)
                    ...
                    logger.info("Successfully processed file : " +
                                infile + " with " + str(trlRowCount) + " lines.")
                elif (trlRowCount == 0):
                    logger.info("Successfully processed file : " +
                                infile + " with " + str(trlRowCount) + " line.")
            else:
...
def lineToDictionary(filename, tenantid, documentType, feedtype, rowkeyschema, rowschema, lineno, line):
    ...
    if (len(rowschema) == len(tokens)):
        ...
        linevalues = {}
        exceptionLinekey = "{0}|{1}|{2}|{3}|{4}|{5}".format(
            tenantid, documentType, "EXCEPTION", feedtype, filename, shortuuid.uuid())
        linevalues['linekey'] = exceptionLinekey
        linevalues['linekeylayout'] = "{0}|{1}|{2}|{3}|{4}|{5}".format(
            "tenantid", "documentType", "EXCEPTION", "feedtype", "filename", "shortuuid")
        linevalues['dictline'] = dictline
...
def insertLine(filename, tenantid, documentType, feedtype, rowkeyschema, rowschema, lineno, line):
    anyException, linevalues = lineToDictionary(
        filename, tenantid, documentType, feedtype, rowkeyschema, rowschema, lineno, line)
    ...
    try:
        ...
        if (e is KeyExistsError):
            logger.debug('insertLine: Exception={3}, FileName={0}, LineNo={1}, Line={2}'.format(
                filename, lineno, line, "RowDuplicateException"))
            raise RowDuplicateException(filename, lineno, line)
        else:
            logger.debug('insertLine: Exception={3}, FileName={0}, LineNo={1}, Line={2}'.format(
                filename, lineno, line, e.__class__))
...
def startErrorReportProcess(tenantid, documentType):
    path = "feeds/"
    DocumentType = documentType
    viewName = "Tenant|{0}|{1}|{2}".format(tenantid, "CUSTOMER", "REJECTED")
    viewDesignName = "{0}_{1}".format("dev", viewName)
    for result in View(startErrorReportProcess.cb, viewDesignName, viewName, include_docs=True):
        logger.info(
            "emitted key: {0}, doc_id: {1}".format(result.key, result.doc.value))
